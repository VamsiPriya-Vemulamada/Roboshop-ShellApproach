...
# The above is a function to check whether the command is successful by getting the value as 0

 PRINT() {
  echo " --------------------  $1 ------------------" >>$LOG
  echo -e "\e[33m$1\e[0m"
 }
#    Below commands are common for all the components and removing if the app is already installed and setting to the specific component name

 LOG=/tmp/$COMPONENT.log
 rm -f $LOG
 set-hostname -skip-apply $COMPONENT

# For downloading each component
  PRINT "Download App Content"
  curl -s -L -o /tmp/$COMPONENT.zip "https://github.com/roboshop-devops-project/$COMPONENT/archive/main.zip" &>>$LOG
  STAT $?
# For setting up the end points of the each component
